...
public class AudioConsumerHandler : IConsumerHandler<Item>
{
    private readonly IEnumerable<IAudioHandler> _audioHandlers;
    private readonly IMonitoringClient _monitoringClient;
    private readonly ILogger<AudioConsumerHandler> _logger;

    public AudioConsumerHandler(
        IEnumerable<IAudioHandler> audioHandlers,
        IMonitoringClient monitoringClient,
        ILogger<AudioConsumerHandler> logger)
    {
    ...
    public async Task HandleMessage(ConsumeResult<string, Item> message, CancellationToken cancellationToken)
    {
        ...
        if (item is null || item.Type != ItemType.Audio && item.Type != ItemType.AudioUrl)
            return;

        try
        {
            await Task.WhenAll(_audioHandlers.Select(handler => handler.HandleAsync(item, cancellationToken)));
        }
        catch (Exception e)
        {

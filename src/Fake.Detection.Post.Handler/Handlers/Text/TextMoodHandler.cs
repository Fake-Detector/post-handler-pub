...
public class TextMoodHandler : ITextHandler
{
    ...
    public async Task HandleAsync(Item item, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var detectorResult = await _fakeTextDetectorService.GetMoodAsync(item.Data, cancellationToken);
